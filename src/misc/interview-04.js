...
console.log(tickets.filter((item) => { if (item.priority === 'normal') return item.priority } ));
...
tasks.forEach((task) => {
  newTickets[task.id] = { title: task.title, priority: convertPriority(task.priority), status: convertStatus(task.status) }
});
